...
/*            
DDL:                
    1. Crear(desde SQL) la Base de Datos y sus Tablas correspondientes de forma detallada y correcta con las siguientes restricciones(CONSTRAINT):
    • PK(Llaves primarias)
    • FK(Llaves foráneas)
    • NOTA: La clausula CHECK es aceptada por MYSQL pero NO vale para nada. En la ACCESS NO es aceptada desde SQL, aunque SI desde el asistente.)
			
    2. Crear los siguientes ficheros índices: IND_PROV  e  IND_PIEZAS en función del NOMBRE y NOMPIEZA respectivamente.
    3. Verificad la creación correcta de las tablas desde el entorno gráfico del Workbench
    4. Aplicad la ingeniería inversa y obtener el modelo ENTIDAD-RELACION desde la base de datos creada(DATA MODELING), verificando y examinando las relaciones existentes entre las distintas tablas. 
    5. Introducid los distintos registros de la tabla  PROVEEDORES mediante la sentencia INSERT(NOTA: probad a introducir un nombre de proveedor duplicado: nooo os debe dejar). 
    6. Introducid el resto de datos de las tablas restantes a través del entorno gráfico del Workbench(EDIT TABLE DATA)
DML:
    7. Probad a borrar el proveedor 3(NO os debe dejar).
    8. Modificar la clave del proveedor 1(en la tabla de Proveedores)  y asignarle el código 8(NO os debe dejar).
*/
...
/# 	   piezas(sin duplicidad). 
	select distinct pr.*
		from proveedores as pr
			inner join pedidos as pd on pd.CODPROV = pr.CODPROV;
...
/#	   de todas las piezas suministradas.
	select pz.codpieza, pz.nompieza,rm.ctd 
		from piezas as pz
			inner join remesas as rm on rm.codpieza = pz.codpieza
...
/#     fecha, codigo proveedor, nombre proveedor, numero pedido.
	...
	select pd.fechap, pr.codprov, pr.nombre, pd.numped
		from pedidos as pd
		inner join proveedores as pr on pd.codprov = pr.codprov
...
/#(ordenados por la fecha del pedido)
    select pd.fechap,pd.numped,rm.codpieza, rm.ctd
		from pedidos as pd
			inner join remesas as rm on rm.NUMPED = pd.NUMPED 
            inner join proveedores as pr on pr.codprov = pd.codprov
...
select pr.codprov, pr.nombre, pr.poblacion, pd.numped, pz.codpieza, rm.ctd
    from proveedores as pr
		inner join pedidos as pd on pd.codprov = pr.codprov 
        inner join remesas as rm on rm.numped = pd.numped 
		inner join piezas as pz on pz.codpieza = rm.codpieza
...
select pr.poblacion, pr.codprov, pr.nombre, pd.fechap, pd.numped, pz.codpieza, pz.nompieza,rm.ctd
    from proveedores as pr
		inner join pedidos as pd on pd.codprov = pr.codprov 
		inner join remesas as rm on rm.numped = pd.numped
		inner join piezas as pz on rm.codpieza = pz.codpieza
			where pd.fechap not between "2021/03/01" 
				and "2021/03/31" 
...
/#	   importe(campo calculado)
	select distinct pd.fechap, pd.numped, pz.codpieza, pz.nompieza, pz.precio, rm.ctd, (rm.ctd*pz.precio)
		from proveedores as pr
			...
			inner join remesas as rm on rm.numped = pd.numped
            inner join piezas as pz on rm.codpieza = pz.codpieza
				where pd.fechap between "2021/01/01" 
					and "2021/04/30"
					and pr.nombre ="FERSA"
    order by pd.fechap, pd.numped;
...
/#codigo pieza, nombre pieza, fecha pedido, precio, cantidad, importe
	select  pz.codpieza, pz.nompieza, pd.fechap, pz.precio, rm.ctd, (rm.ctd*pz.precio)
		from piezas as pz 
			inner join remesas as rm on rm.codpieza = pz.codpieza
			inner join pedidos as pd on pd.numped = rm.numped
				where pd.fechap between "2021/01/01" 
					and "2021/01/31"
					and ((rm.ctd*pz.precio)>1000)
	order by pz.codpieza;
...
/#37.- Obtener el importe total de los pedidos de Marzo.
    select sum(rm.ctd*pz.precio) as importePedMarzo
		from remesas as rm
			inner join piezas as pzon pz.codpieza = rm.codpieza
			inner join pedidos as pd on pd.NUMPED = rm.numped
				where pd.fechap between "2021/03/01" 
					and "2021/03/30";
...
/#	 (codigo proveedor, nombre prov, numeropedidos)
	select pr.codprov, pr.nombre, count(pd.numped)
		from proveedores as pr
			inner join pedidos as pd on pr.codprov = pd.codprov
        group by pr.codprov 
...
SELECT  pd.fechap, pd.numped, pz.codpieza, rm.ctd 
	from remesas as rm
		inner join pedidos as pd on pd.numped = rm.numped
		inner join piezas as pz on pz.CODPIEZA = rm.CODPIEZA
			where pd.codprov = (select  pr.codprov
									from proveedores  as pr
										where pr.nombre= "Fersa");
...
/#57.- Obtener el codigo pieza, nombre pieza  y  peso de mayor peso.
	select pz.codpieza, pz.nompieza, pz.peso
		from piezas as pz 
			where pz.peso=(	select max(pz.peso)
				from piezas as pz);
